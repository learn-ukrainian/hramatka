...
checkout walk or a dev-home path:
  ...
  - the AIS secret       -> `HRAMATKA_AIS_API_KEY` or the file named by
                            `HRAMATKA_AIS_API_KEY_FILE` (see `engine.transport`)
...
from .transport import AIS_API_KEY_FILE_ENV, AISGeneratorPort, GeneratorUnavailable
...
def preflight(*, require_generator: bool = False) -> None:
    ...
    Checks (fail fast, all failures reported at once):
      ...
      - an AIS key source resolves — ONLY when `require_generator=True` (offline
    ...
    if require_generator:
        try:
            AISGeneratorPort(api_key_file_env=AIS_API_KEY_FILE_ENV).resolve_key()
