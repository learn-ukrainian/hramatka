"""Offline capture of the #552 prospective 64-cell loss grid."""
...
def _verified_engine_bundle(bundle: data.DataBundle, repository: Path) -> data.DataBundle:
    """Freeze the PR #547 engine inputs separately from prospective source text."""
    ...
    try:
        ...
        raise ProspectiveCaptureError("Frozen engine authority cannot be read locally.") from exc
    inputs = bundle.manifest.get("inputs")
    if not isinstance(inputs, dict) or set(inputs) != set(policy):
        raise ProspectiveCaptureError("Engine bundle does not match frozen PR #547 authority.")
    for name, row in policy.items():
        ...
        if not isinstance(actual, dict) or any(actual.get(key) != row[key] for key in row):
            raise ProspectiveCaptureError("Engine bundle policy does not match PR #547 authority.")
    ...
    try:
        ...
        raise ProspectiveCaptureError("Engine bundle has drifted.") from exc
    return detached


def _source_dict(source: ProspectiveSource) -> dict[str, object]:
    return source.to_dict()


def _candidate_banks(receipts: tuple[CandidateBankReceipt, ...]) -> list[dict[str, object]]:
    ...
    if profile.digest != PROFILE_DIGEST or density_floor_fingerprint() != DENSITY_FLOOR_DIGEST:
        raise ProspectiveCaptureError("Existing profile or density floor authority has drifted.")
    return rows
...
def _without_pair(
    ...
    return replace(
        ...
        candidates=tuple(
            ...
            if candidate.candidate_id not in {quiz_id, cloze_id}
        ),
    )
...
def _allocation_projection(allocation: LessonAllocation) -> dict[str, object]:
    ...
    for slot in allocation.slots:
        requested = profile.tier_for(slot.requested_type)
        scheduled = profile.tier_for(slot.scheduled_type)
        if _TIER_RANK[scheduled] < _TIER_RANK[requested]:
            raise ProspectiveCaptureError(
        ...
        if (
            ...
            raise ProspectiveCaptureError("Open response or extended writing was replaced.")
        rows.append(
            {
    ...
    if len(rows) != 6:
        raise ProspectiveCaptureError("Existing six-slot authority has drifted.")
    try:
        validate_phase_pace(allocation.slots)
    except ValueError as exc:
        raise ProspectiveCaptureError("Existing phase-pace authority has drifted.") from exc
    return {
...
def _witness(allocation: LessonAllocation) -> dict[str, object]:
    projection = _allocation_projection(allocation)
    return {
        "rows": [
            {
                "slot_id": row["slot_id"],
...
def _cell(
    ...
    result = preflight_lesson(
        AnchorWindow(
            (AnchorParagraph(str(source.sqlite_id), _without_pair(inventory, quiz_id, cloze_id)),),
            0,
            0,
        ),
        duration_minutes=45,
    ...
    allocation = _allocation_projection(result.allocation)
    return {
        "identity": identity,
        "outcome": "passed",
...
def capture_source(
    ...
    *,
    ...
    """Capture all 64 exclusions, preserving an infeasible source as a failure."""
    ...
    try:
        with data.use_bundle(engine_bundle):
            inventory = inventory_from_anchor(
                ...
                receipt_context=BankReceiptContext(
                    manifest_digest=manifest.digest, source_identity=str(source.sqlite_id)
                ),
        ...
        banks = _candidate_banks(receipts[0])
    except (ValueError, ProspectiveSourceInfeasible) as exc:
        return ProspectiveCertificate(
            {
    ...
    with data.use_bundle(engine_bundle):
        cells = [
            _cell(inventory, source, receipts[0], quiz_id, cloze_id)
            for cloze_id in memberships["cloze:1"]
            for quiz_id in memberships["quiz:1"]
        ]
    failed = [row for row in cells if row["outcome"] != "passed"]
    return ProspectiveCertificate(
        {
            "version": "HramatkaProspectiveCertificate.v1",
            "manifest_digest": manifest.digest,
